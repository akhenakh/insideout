...
var (
	...
	insideMaxLevelCover  = flag.Int("insideMaxLevelCover", 17, "Max s2 level for inside cover")
	insideMinLevelCover  = flag.Int("insideMinLevelCover", 10, "Min s2 level for inside cover")
	insideMaxCellsCover  = flag.Int("insideMaxCellsCover", 64, "Max s2 Cells count for inside cover")
...
func main() {
	...
	storage, clean, err := sbbolt.NewOSMStorage(*dbPath, logger)
	if err != nil {
