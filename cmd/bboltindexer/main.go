...
func main() {
	...
	err = storage.Update(func(tx *bbolt.Tx) error {
		if _, err := tx.CreateBucket([]byte("info")); err != nil {
			return err
		}
		if _, err := tx.CreateBucket([]byte("feature")); err != nil {
			return err
		}
		if _, err := tx.CreateBucket([]byte("cell")); err != nil {
			return err
		}
		return nil
	})
