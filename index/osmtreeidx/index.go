...
func (idx *Index) Add(cellsIn []s2.CellUnion, cellsOut []s2.CellUnion, id int64) {
	for i, cu := range cellsIn {
		for _, c := range cu {
			idx.itree.Index(c, insideout.OSMFeatureIndexResponse{
				ID:  id,
				Pos: uint16(i),
			})
		}
	...
	for i, cu := range cellsOut {
		for _, c := range cu {
			idx.otree.Index(c, insideout.OSMFeatureIndexResponse{
				ID:  id,
				Pos: uint16(i),
			})
		}
...
func (idx *Index) Stab(lat, lng float64) (insideout.OSMIndexResponse, error) {
	var idxResp insideout.OSMIndexResponse
	...
	for _, r := range res {
		fres := r.(insideout.OSMFeatureIndexResponse)
